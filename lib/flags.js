// Flag emoji, country names and toolbar icon rendering, including the red
// dot that marks a country change the user has not looked at yet.
...
// Same red as the leak badge, so the icon speaks with one voice.
const DOT_COLOR = '#d93025';

// The unread mark: a red dot in the top-right corner. A ring around it is cut
// out to transparent rather than painted white, so the dot stays separate from
// a flag that is itself red - and from the toolbar, whatever its theme.
 ...
 * width, or a country-code badge when flag emoji are unavailable. With `mark`
 * set, a red dot sits in the corner - the sign of a country change that has
 * not been looked at.
 */
export function drawIcon(cc, size, mark = false) {
...
export function iconImageData(cc, mark = false) {
  const key = `${cc ?? '??'}${mark ? '+dot' : ''}`;
  let icons = iconCache.get(key);
  if (!icons) {
    icons = {};
    for (const size of [16, 24, 32, 48]) icons[size] = drawIcon(cc, size, mark);
