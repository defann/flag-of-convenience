...
export function consensus(results) {
  ...
  if (!ok.length) {
    return {
      ...
      staleSockets: false, secure: false,
    };
  }
  ...
  return {
    ...
    // At least one https source stands behind the winner. An answer fetched in
    // the clear can have been rewritten on the way, so a country that only such
    // an answer reports is never acted on - see the promotion rule in
    // background.js.
    secure: cc !== null && ok.some((r) => r.cc === cc && r.secure !== false),
  };
}
