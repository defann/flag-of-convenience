...
// down for hours at a time, and an exit IP behind a VPN pool can differ
// between two requests made a second apart.
//
...
//
// Every source is kept on a server that speaks HTTP/1.1, so its connection can
// be dropped after each check - see dropConnections() for why that matters.
//
...
// it on HTTP/1.1. Anything reached in the clear is marked insecure here, and a
// country that only such an answer reports never moves the icon - see the
// promotion rule in background.js.
...
export const SOURCES = [
  ...
  },
  {
    ...
    // rewritten on the way, which is why a reading from here never moves the
    // country on its own - see the promotion rule in background.js.
  ...
  {
    id: 'checkip.now',
    url: 'https://api.checkip.now/',
    // Serves https over HTTP/1.1 on purpose (its ALPN offers nothing else), so
    // its connection can be dropped after every check like seeip.org's. A third
    // witness is what lets a round settle a disagreement between the other two
    // by majority instead of waiting for the next check.
    http1: true,
...
// What a resting source is remembered by: its answer from the last round it
// took part in, whether a reading or a failure, carried on from round to round.
function lastReading(before) {
  if (!before) return null;
  if (before.skipped) return before.last ?? null;
  return { ip: before.ip, cc: before.cc, error: before.error ?? null };
}
 ...
 * dropped, so the popup can still account for all of them, and it carries the
 * answer it gave last time (looked up in `previous`, the sources of the last
 * check) under `last` - never under ip/cc, since a resting source must not
 * vote on a reading that may predate a network change.
 */
export function probeAll(fetchImpl, shouldProbe, previous = []) {
  return Promise.all(SOURCES.map((s) => (
    !shouldProbe || shouldProbe(s)
      ...
      : {
        ...
        last: lastReading(previous.find((p) => p?.id === s.id)),
      }
  )));
}
...
// and leaves the session open - which is why every source is deliberately kept
// on a server that speaks HTTP/1.1.
